...
import { toNumber, getNearestHour } from "./util.ts";
...
export const snapshot = async (ctx: {
	block: Block,
	client: PublicClient,
	store: Store,
  timestamp: number,
  ...
  const { block, client, store, timestamp, pool, liquidity } = ctx
  const ohlc = await OhlcUtil.getOrGapFill(client, store, timestamp, pool.address)
  if (!ohlc) {
    console.log('no ohlc')
    return null
  }
  ...
  return new Snapshot({
    ...
    liquidity: liquidity ? toHex(liquidity) : toHex(0),
    low: ohlc.low,
    high: ohlc.high,
    totalValueLockedUSD,
...
export const hourDataHandler: BlockHandler = async ({ block, client, store }: {
	...
	if (lastHour < nowHour) {
      ...
      return snapshot({ block, client, store, timestamp: nowHour, pool })
		}))).filter(e => e !== null)
