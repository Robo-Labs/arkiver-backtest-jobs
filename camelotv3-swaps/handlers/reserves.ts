...
export const getReserves = async (client: PublicClient, store: Store, blockNumber: bigint, address: Address) => {
  const id = `Univ3:Reserves:${address}`
  const reserves = await store.get(id) as Reserves | undefined
  if (!reserves) {
