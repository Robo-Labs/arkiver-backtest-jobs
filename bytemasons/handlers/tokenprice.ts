...
import { velodromeV1Router } from "../abis/velodromeV1Router.ts";
import { velodromeV2Router } from "../abis/velodromeV2Router.ts";
...
const VELOV2FACTORY = '0xF1046053aa5682b4F9a81b5481394DA16BE5FF5a'
const VELODROME_ROUTERV2 = '0xa062aE8A9c5e11aaA026fc2670B0D65cCc8B2858'

export class TokenPrice {

	static async get(client: PublicClient, store: Store, block: bigint, token: any, stable: boolean): Promise<number> {
		return await store.retrieve(`TokenPrice:${token.address}:${Number(block)}`, async () => {
			try {
				...
				return await TokenPrice.getVelodromeV2SpotPrice(client, block, token, stable)
			}
		})
	}
	...
	static async getVelodromeV1SpotPrice(client: PublicClient, block: bigint, token: any) {
		//const VELODROME_ROUTER = "0x9c12939390052919aF3155f41Bf4160Fd3666A6f"
		const swapAmount = 10 ** token.decimals
		const amountOut = await client.readContract({
			abi: velodromeV1Router,
			address: VELODROME_ROUTER,
			functionName: "getAmountOut",
			args: [BigInt(swapAmount), token.address, USDC ],
			blockNumber: block,
		})
	...
	static async getVelodromeV2SpotPrice(client: PublicClient, block: bigint, token: any, stable: boolean) {
		//const VELODROME_ROUTER = "0x9c12939390052919aF3155f41Bf4160Fd3666A6f"
		const swapAmount = 10n ** BigInt(token.decimals)
		// console.log(`getVelodromeV2SpotPrice swapAmount: ${swapAmount}`)
		// console.log(`getVelodromeV2SpotPrice token: ${token.address}`)
		// console.log(`getVelodromeV2SpotPrice stable: ${stable}`)
		const amountOut = await client.readContract({
			abi: velodromeV2Router,
			address: VELODROME_ROUTERV2,
			functionName: "getAmountsOut",
			args: [swapAmount, [[token.address, USDC, stable, VELOV2FACTORY]]],
			blockNumber: block
		})
		//return 0
		return toNumber(amountOut[1], 6)
	}

	static async getVelodromeV2SpotPriceInToken(client: PublicClient, block: bigint, base: any, quote: any, stable: boolean) {
		const swapAmount = 10n ** BigInt(base.decimals)
		console.log(`getVelodromeV2SpotPrice swapAmount: ${swapAmount}`)
		console.log(`getVelodromeV2SpotPrice token: ${base.address}`)
		console.log(`getVelodromeV2SpotPrice stable: ${stable}`)
		const amountOut = await client.readContract({
			abi: velodromeV2Router,
			address: VELODROME_ROUTERV2,
			functionName: "getAmountsOut",
			args: [swapAmount, [[base.address, quote.address, stable, VELOV2FACTORY]]],
			blockNumber: block
		})
		//return 0
		return toNumber(amountOut[1], 6)
